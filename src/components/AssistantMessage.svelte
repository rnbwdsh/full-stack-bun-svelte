                ...
                <details open>
                    ...
                    <div class={$settings.rotate2 ? "flex1" : "flex1c"} >
                        {#each message.other[model] as responses_model}
                            <button class="bubble-chat bubble-inner" on:dblclick={setAnswer} value={responses_model.map(r => r.content).join("")}>
...
<style>
    .bubble-chat {
        text-align: left;
        align-self: start;
        background-color: color-mix(in lch, var(--pico-primary-background) 60%, var(--pico-background-color));
